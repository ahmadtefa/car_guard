...
void main() {
  group('license message parser', () {
    ...
    test('fresh status maps to distinct data-authorization states', () {
      const active = LicenseStatusMessage(
        status: LicenseDeviceStatus.active,
        licenseType: LicenseType.temporary,
        expires: 2_000_000_000,
      );
      const missing = LicenseStatusMessage(
        status: LicenseDeviceStatus.locked,
        licenseType: LicenseType.none,
        expires: 0,
      );
      const expired = LicenseStatusMessage(
        status: LicenseDeviceStatus.locked,
        licenseType: LicenseType.temporary,
        expires: 1_000_000_000,
      );
      const invalid = LicenseStatusMessage(
        status: LicenseDeviceStatus.locked,
      ...
      expect(
        licenseCheckStatusFor(active),
        LicenseCheckStatus.licensed,
      );
      expect(
        licenseCheckStatusFor(missing),
        LicenseCheckStatus.noLicense,
      );
      expect(
        licenseCheckStatusFor(
          expired,
    ...
    test('reason mapping covers firmware strings with a generic fallback', () {
      ...
      expect(
        licenseFailureReasonFromFirmware('INVALID_DATE'),
