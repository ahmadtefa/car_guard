...
class GpsTripFilter {
  ...
  TripReading? addFix(Position position) {
    ...
    // A trusted sensor is authoritative — including a trusted 0 while
    // parked. Never override a trusted reading with track-derived motion,
    // or parked GPS jitter would accumulate phantom distance. The
    // track-derived fallback below applies only to an *untrusted*
    // (dead/absent) sensor, which _sensorSpeedMs reports as -1.
    if (sensorMs >= 0) {
