...
abstract final class AnalysisEngine {
  ...
  // ------------------------------------------------------------------
  // Smart alert tracking: cooldown + escalation without spamming
  // ------------------------------------------------------------------
  ...
  class SmartAlertTracker {
    ...
    TrackerEvent? update(
      ...
      void Function(AnalysisAlert alert, TrackerEvent event)? onEvent,
    }) {
      ...
      if (!condition) {
        ...
        if (now.difference(falseSince) >= clearAfter) {
          ...
          onEvent?.call(current, TrackerEvent.cleared);
          return TrackerEvent.cleared;
        }
        return null;
      }
      ...
      if (current == null) {
        ...
        active[kind] = alert;
        onEvent?.call(alert, TrackerEvent.started);
      ...
      if (sinceLastFire >= cooldown) {
        ...
        onEvent?.call(escalated, TrackerEvent.escalated);
        return TrackerEvent.escalated;
      }
