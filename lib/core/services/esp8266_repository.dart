...
class Esp8266Repository implements DeviceRepository {
  ...
  // عداد لتقليل الـ spam في الـ logs - يطبع كل 10 ثواني فقط عند تفعيل الـ verbose
  int _httpTickCounter = 0;
  static const bool _verboseLogs = false; // غيّرها لـ true لو عايز تشوف كل رسالة للتصحيح
  ...
  void _logHttpData(String body) {
    // في الوضع العادي لا تطبع كل ثانية - اطبع كل 10 ثواني فقط ولو الـ verbose شغال
    if (!_verboseLogs) return;
    if (!kDebugMode) return;
    _httpTickCounter++;
    if (_httpTickCounter % 10 == 0) {
      debugPrint("HTTP DATA = $body");
    }
  }
  ...
  Future<void> connect({
    ...
    _log("TRY WEBSOCKET ws://$host:$wsPort");
  ...
  void _startHttpFallback(String host) {
    if (_usingHttpFallback) {
      return;
    }

    _log("START HTTP FALLBACK");
      ...
      if (data.trim().startsWith('{')) {
        ...
        status = DeviceStatus(
          ...
          temperatureData: TemperatureData(
            engineTemperature: (json["temp"] as num).toDouble(),
          ),
          coolantLevelData: const CoolantLevelData(
            coolantAvailable: true,
          ),
          controlData: DeviceControlData(
            fanRunning: json["fanState"] == 1,
            buzzerActive: json["buzzerState"] == 1 ||
        ...
        status = DeviceStatus(
          ...
          controlData: DeviceControlData(
            ...
            buzzerActive: parts.length > 4 ? parts[4].trim() == "1" : false,
          ),
          lastUpdated: DateTime.now(),
        );
      }
  ...
  Future<void> sendJson(Map<String, dynamic> payload) async {
    ...
    try {
      ...
      _log("HTTP SEND ERROR $e");
    }
  }

  @override
  Stream<DeviceStatus> get liveUpdates => _statusController.stream;
