...
enum LicenseCheckStatus {
  ...
  /// The current ESP8266 session reported an active license.
  licensed,

  /// The module reported a temporary license that has ended.
  expired,

  /// The module reported a license state that is not usable, or activation
  /// failed because the supplied license was invalid.
  invalid,

  /// The module explicitly reported that it has no license.
  noLicense,

  /// The module could not be reached or did not answer the license query.
  error,
}

/// Maps a fresh authoritative [LicenseStatusMessage] to the UI/data state.
///
/// A firmware `ACTIVE` response always wins: the ESP8266 remains the authority
/// and Flutter never re-validates or trusts a cached expiry on its own. For a
/// `LOCKED` response, the protocol's license type identifies the useful reason
/// without changing the wire format.
LicenseCheckStatus licenseCheckStatusFor(
  LicenseStatusMessage message, {
  DateTime? now,
}) {
  if (message.status == LicenseDeviceStatus.active) {
    return LicenseCheckStatus.licensed;
  }

  switch (message.licenseType) {
    case LicenseType.none:
      return LicenseCheckStatus.noLicense;
    case LicenseType.temporary:
      // A LOCKED temporary license is expired according to the authoritative
      // module. The timestamp is only used to distinguish a malformed locked
      // response; it is never used to grant access.
      final current = now ?? DateTime.now().toUtc();
      final nowEpoch = current.millisecondsSinceEpoch ~/ 1000;
      return message.expires > 0 && message.expires > nowEpoch
          ? LicenseCheckStatus.invalid
          : LicenseCheckStatus.expired;
    case LicenseType.permanent:
      return LicenseCheckStatus.invalid;
  }
}
...
LicenseFailureReason licenseFailureReasonFromFirmware(String? reason) {
  ...
  switch (reason.trim().toUpperCase()) {
    ...
    case 'INVALID_DATE':
      return LicenseFailureReason.invalidDate;
...
/// Converts an activation failure into the data-gate state without exposing
/// raw firmware details. Transport/time/storage/configuration failures remain
