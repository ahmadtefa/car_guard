...
The car integration is declared with the `com.google.android.gms.car.application`
meta-data + `res/xml/automotive_app_desc.xml` and uses the `IoT` app category,
which is the official fit for device-monitoring apps. See
[docs/android_auto.md](docs/android_auto.md) for sideload/developer-mode
testing and Play Store notes.
...
upload — it can never change after), for a Play upload pick the `IoT`
Android Auto category (or drop the car service if you don't want to ship
Android Auto), and a release keystore is required.
